...
def PredictWithData(data,data_yuan,name,model,modelname,INPUT_DIMS = 13,TIME_STEPS = 20):
    print(data.columns)
...
'''def xgboost_forecast(train, testX):
    ...
    trainX = trainX[:,:-5]
    pd.DataFrame(trainX).to_csv('TrainX_XGboost.csv')
    pd.DataFrame(trainy).to_csv('Trainy_XGboost.csv')
...
def xgboost_forecast(train, testX):
    ...
    # make a one-step prediction
    v=testX.iloc[:-5]
    yhat = model.predict(np.asarray([v]))
    return yhat[0]
